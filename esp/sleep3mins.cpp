...
#define SERVER_URL "http://192.168.1.7/weevibesv2/model/data.php"
...
#define RECORD_INTERVAL 180000  // 3 minutes in milliseconds
...
// Main loop
void loop() {
    Serial.println("Starting recording cycle...");

    // **Update lastRecordTime BEFORE recording starts**
    lastRecordTime = millis();  // 🛠 **Fix: Store start time of the cycle**
    ...
    // **Enter light sleep until next recording cycle**
    Serial.println("Entering light sleep...");

    // Calculate remaining sleep time
    unsigned long elapsedTime = millis() - lastRecordTime;
    unsigned long sleepTime = (elapsedTime < RECORD_INTERVAL) ? (RECORD_INTERVAL - elapsedTime) : 0;
    ...
    Serial.println("Woke up from light sleep! Starting new cycle...");
}
